...
S9_StorageLevelSeasonStart(r,s,ls,y)$(ord(ls) = 1)..
...
S11_StorageLevelDayTypeStart(r,s,ls,ld,y)$(ord(ld) = 1)..
...
SI7_SalvageValueStorageAtEndOfPeriod2(r,s,y)$((DepreciationMethod(r)=1 and (y.val+OperationalLifeStorage(r,s)-1) > smax(yy,yy.val) and DiscountRate(r)=0) or (DepreciationMethod(r)=2 and (y.val+OperationalLifeStorage(r,s)-1) > smax(yy,yy.val)))..
    CapitalInvestmentStorage(r,s,y)*(1- sum(yy$(ord(yy)=card(yy)),yy.val)  - y.val+1)/OperationalLifeStorage(r,s) =e= SalvageValueStorage(r,s,y);
...
SV1_SalvageValueAtEndOfPeriod1(r,t,y)$(DepreciationMethod(r)=1 and ((y.val + OperationalLife(r,t)-1 > smax(yy, yy.val)) and (DiscountRate(r) > 0)))..
...
SV2_SalvageValueAtEndOfPeriod2(r,t,y)$(((y.val + OperationalLife(r,t)-1 > smax(yy, yy.val)) and (DiscountRate(r) = 0)) or (DepreciationMethod(r)=2 and (y.val + OperationalLife(r,t)-1 > smax(yy, yy.val))))..
