...
%figure; plot(years,cum,'linewidth',2); 
%grid on; xlabel('years'); ylabel('commulative %'); title('% annual production by individual technology'); legend(labels); axis([2015 2050 0 100]);
%pieplot 2050
explode=[1 1 1 0 0 0 0 0 0];
figure; pie(per(:,end),explode); legend(labels);

%emission
emit= M(755, 2:37)/1000; %in Million tons
figure; plot(years,emit,'linewidth',2); 
grid on; xlabel('years'); ylabel('Emissions (Million tons CO2))'); 
